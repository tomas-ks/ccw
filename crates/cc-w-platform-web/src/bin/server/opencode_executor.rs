...
fn build_prompt(request: &OpencodeTurnRequest, native_agent: bool) -> String {
    let prompt = vec![
        ...
        "- For ad hoc Cypher, do not freestyle from scratch if a playbook would fit. Ask for `get_query_playbook`, then adapt the recommended pattern minimally to the current question.".to_owned(),
        ...
        "- For broad material questions like `what is the house built of`, avoid schema thrash. Start with a query playbook or a small live material query such as `MATCH (:IfcRelAssociatesMaterial)--(material:IfcMaterial) RETURN DISTINCT material.Name AS material_name LIMIT 24`, and only refine from there if the result is too thin.".to_owned(),
